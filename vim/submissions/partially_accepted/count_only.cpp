// Derived from the organizers' koci_37_greedy2.cpp by deleting the loop that
// prints the command sequence, so it prints only the (correct) minimum command
// count. Exercises the statement's "half the points for that subtask" rule.
// Expected score: 50 (half of every group).
