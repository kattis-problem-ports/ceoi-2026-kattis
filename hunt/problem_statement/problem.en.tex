...
at the top of your program. The library exists only for C++, so this task can only
be solved in C++.
...
you can use while developing your solution. To compile the library together with
your program, simply add its name as a parameter to the compiler, e.g.
...
own random treasure hunts. It is not declared in the header file, so declare it
yourself where you use it:

\begin{verbatim}
extern void InitFromFile(const char *fileName);
\end{verbatim}

The comments at the end of \texttt{treasurehuntlib-public.cpp} describe the file
format; the attachment \texttt{sample.in} is such a file, holding the single
treasure hunt of the example below.
...
\section*{Example}
The table below shows one treasure hunt on a $4 \times 4$ grid holding a single
chest, which sits at $(3, 2)$: the calls your program makes into the library and
the values the library gives back. None of it passes through standard input or
standard output, these are ordinary function calls, so there is no sample input or
output and the table is the whole interaction. The attachment \texttt{sample.in}
describes this very hunt, so \texttt{InitFromFile} will replay it for you locally.
...
\begin{tabular}{| l | l |}
  ...
  \texttt{Query(3, 1)}    & \texttt{DIR\_DOWN} $= 8$ \\ \hline
  \texttt{Query(3, 2)}    & \texttt{TREASURE} $= 0$ \\ \hline
